...
.err-code-big span { color: ${accentColor}; }
.err-headline { font-size: clamp(22px, 3vw, 32px); font-weight: 800; color: #fff; margin: 20px 0 14px; letter-spacing: -0.5px; line-height: 1.2; }
.err-desc { font-size: 15px; color: rgba(255,255,255,0.5); line-height: 1.7; max-width: 380px; }
...
.err-btn-primary {
  ...
  cursor: pointer; text-decoration: none; transition: all 0.15s; width: 100%; margin-bottom: 10px;
}
...
function ErrorPage({ statusCode }) {
  ...
  const accentColor = is404 ? "#16a34a" : "#ef4444";

  const codeStr = String(statusCode || "Err");
  const mid = Math.floor(codeStr.length / 2);

  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: getCSS(accentColor) }} />
      <div className="err-root">
        ...
        <div className="err-left">
          ...
          <div className="err-code-big">
            {codeStr.split("").map((d, i) =>
              i === mid ? <span key={i}>{d}</span> : <React.Fragment key={i}>{d}</React.Fragment>
            )}
          </div>
